...
export function MasteryLevelIcon({ level, className = '', style }: MasteryLevelIconProps) {
  const normalizedLevel = Math.min(9, Math.max(0, Math.floor(level)));
  const label = normalizedLevel === 0 ? '尚未学习' : `学习等级 ${normalizedLevel}`;
  const imageUrl = normalizedLevel > 0
    ? `${import.meta.env.BASE_URL}content/images/ui/mastery-levels/level-${normalizedLevel}.webp?v=2`
    : null;

  return (
    <span
      className={`mastery-level-icon mastery-level-icon--level-${normalizedLevel}${className ? ` ${className}` : ''}`}
      role="img"
      aria-label={label}
      title={label}
      style={style}
